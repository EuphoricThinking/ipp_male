...
size_t* convert_to_size_t_array(char* read_input, size_t* length_after_processing) {
    ...
    size_t size_allocated = sizeof(size_t);
    size_t current = 0;
    ...
    while ((converted = (size_t) strtoull(to_pass, &next_string, BASE))) {
        ...
        if (current == size_allocated) {
            size_allocated = 2*current;
            err = realloc(number_array, size_allocated);
            if (!err) {
                printf("BBBB\n");
                print_error(ERR_0);
                exit(1);
...
//        exit(1);
    ...
    size_t* end_coordinates = convert_to_size_t_array(workline, &read_numbers);
